                ...
                <div class="card-body">
                    <form method="post" enctype="multipart/form-data" action="{{route('post.store', $thread-> id)}}">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-8">
                                <textarea id="post_content" rows="15" class="form-control @error('description') is-invalid @enderror" name="post_content" required><blockquote class="px-1 ml-3 mr-1 border">{{ __('Citation de') }}<b>{{ $quotedmessage -> user-> name}}</b>
                                <p>{{$quotedmessage->post_content}}<p></blockquote></textarea>
                            </div>
                        </div>
